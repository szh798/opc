...
**版本**: V3.0（MVP简化版：8个chatflow + 零AI路由 + 主人格为核心日常体验）  
**日期**: 2026年4月  
...
| 路由机制   | MVP：纯数据库字段检查（20行Python，零AI）。V2：复用三层路由架构 |
...
> **to 李金芮：MVP阶段的路由极度简单——纯数据库字段检查，零AI参与。**

## 3.1 MVP路由：一个if-else（零AI成本）

MVP阶段用户只有两种状态："还没做完盘点"和"做完了"。做完了之后只有一个地方可去——种子主人格。不需要关键词匹配，不需要意图分类，不需要LLM路由。

```
用户进入
  │
  ▼
onboarding_completed = false? ──YES──► 首次登录Onboarding
  │ NO                                 （Step 1-4 → 深度访谈 → 雷达图）
  ▼
last_incomplete_flow != null? ──YES──► 恢复未完成的盘点板块
  │ NO
  ▼
has_talent_radar = false? ──YES──► 引导完成盘点
  │ NO
  ▼
radar_update_pending = true? ──YES──► 种子主人格（开场展示雷达图更新）
  │ NO
  ▼
种子主人格 zhongzi_main_chat（唯一的日常chatflow）
```

> **to 李金芮：Agent Router API的完整实现：**
...
def route_user(user_id):
    ...
    if not user.onboarding_completed:
        return { "chatflow_id": "onboarding_flow", "mode": "fixed" }
    
    # 有未完成的盘点板块
    if user.last_incomplete_flow:
        return { 
            "chatflow_id": user.last_incomplete_flow, 
            "mode": "fixed",
            "resume_step": user.last_incomplete_step 
        }
    
    # 盘点未完成（异常情况）
    if not user.has_talent_radar:
        return { "chatflow_id": "talent_audit_block1", "mode": "fixed" }
    
    # 所有正常情况 → 种子主人格
    greeting = generate_greeting(user)
    return { 
        "chatflow_id": "zhongzi_main_chat", 
        "mode": "free",
        "greeting": greeting 
    }
```

> **to 史志恒：MVP阶段前端不需要实现角色切换下拉菜单、侧边栏角色选择、关键词路由表。只需要：对话主屏 + 开场白渲染 + 卡片渲染（雷达图/更新提示）。角色切换UI可以做占位但不需要实际功能。**

## 3.2 【V2未来迭代】三层路由架构

> **以下设计在引路（种子·引路）和发现自由聊天（种子·发现）上线后才需要实现。MVP阶段跳过。**

当多个子智能体chatflow上线后，用户在主人格里说"帮我看看这周做什么任务"时需要路由到引路，说"我想重新盘点一下"时需要路由到发现。届时启用三层路由：

| 层级 | 机制 | 延迟 |
|------|------|------|
| 第一层 | 快捷回复按钮绑定chatflow ID | 0ms |
| 第二层 | 前端关键词规则匹配 | <10ms |
| 第三层 | 轻量意图分类（小模型或极简LLM） | <200ms |

关键词规则表、意图分类方案的详细设计见V2版本PRD存档。MVP阶段不开发。
...
| 家长说"太多了，我没时间"     | "我们已经聊了X/5个话题了。剩下的部分每个大概5分钟。聊完你就能看到完整的天赋雷达图——很多家长看完都说'原来我的孩子有这么多被忽略的天赋'。"               |
...
## 模块2：成长路径规划（发现 主导）【V2未来迭代——MVP阶段由种子主人格在对话中自然完成】
...
## 模块3：每周成长任务（引路 主导）【V2未来迭代——MVP阶段由种子主人格在对话中临时给出活动建议】
...
## 模块4：微项目系统（引路 主导，7岁+才开启）【V2未来迭代——MVP阶段不开发】
...
## 模块5：种子主人格 — MVP核心日常体验【P0 MVP核心】

> **MVP阶段，种子主人格是家长完成天赋盘点后唯一的日常对话对象。它不是"兜底"，它是产品的主力体验。详细设计见独立文档《种子主人格_MVP核心Chatflow详细设计_V1.md》。以下为摘要。**

### chatflow ID: zhongzi_main_chat
### LLM节点数: 1
### 触发条件: has_talent_radar = true（即完成天赋盘点后的所有日常场景）

### MVP阶段承担的5个职责（按优先级）
1. **焦虑缓释**（最高优先）— 检测到家长焦虑时先缓解再给建议
2. **育儿问题回答** — 基于孩子天赋档案给个性化建议（全脑教养法+PERMA+Grit）
3. **天赋信号持续采集** — 在日常对话中自然提取孩子新信号，静默写入memory
4. **成长建议** — 替代未上线的引路功能，在对话中自然给出活动建议（SDT三需求检查）
5. **闲聊陪伴** — 不是所有对话都要有教育意义
...
child_age < 3  → 种子说"3岁以下宝宝还在感官发育期。我先给你一些这个阶段的建议。"
                 → 进入zhongzi_main_chat（早期养育模式）。不做天赋盘点。
...
> **to 李金芮：MVP阶段不需要前端关键词匹配或意图分类。Onboarding只有一个LLM节点，它自己就能处理自由输入——在system prompt中加入规则："如果家长不点快捷回复而是直接打字，你的任务是：回应内容→自然追问孩子信息→引导到'他叫什么名字'。"**

所有自由输入最终都汇聚到同一个地方：种子回应家长说的内容，然后自然地追问孩子的信息。不需要路由，因为onboarding阶段只有一个目标——收集到孩子名字和年龄，然后进入深度访谈。
...
> **to 李金芮：MVP阶段的再次登录路由与第三章的状态机完全一致——就是那个if-else。此处补充开场白逻辑。**

## 6.0 入口决策树（与第三章相同，此处为快速参考）

```
Step 1: onboarding_completed = false? → 首次登录流程
Step 2: last_incomplete_flow != null? → 恢复未完成盘点板块
Step 3: has_talent_radar = false? → 引导完成盘点
Step 4: radar_update_pending = true? → 种子主人格（开场展示雷达图更新）
Step 5: → 种子主人格（正常日常对话）
```
...
def generate_greeting(user):
    ...
    if user.radar_update_pending:
        changes = get_radar_changes(user.id)  # 获取变化的维度和触发信号
        return f"有个发现要告诉你。{changes.trigger_summary}——" \
               f"{child}的{changes.dimension}在上升。雷达图已经更新了。"
    ...
    # 优先级4：默认
    return f"来了！{child}最近有什么想聊的？"
```

> **to 史志恒：开场白不需要快捷回复按钮。主人格是自由对话模式。如果有雷达图更新，开场白后面跟一个雷达图卡片（变化维度高亮）。**

## 6.2 【V2未来迭代】用户主动选择角色

> **以下在引路和发现自由聊天上线后才需要实现。MVP阶段角色切换UI做占位但不实现功能。**

| 角色 | 数据库检查 | 无成果物→ | 有成果物→ |
|------|-----------|----------|----------|
| 种子·发现 | has_talent_radar? | 进入盘点 | 自由聊天（"有新变化吗？"）|
| 种子·引路 | has_talent_radar? | "先跟发现聊聊" | 任务反馈+新任务生成 |

---

# 第七章：记忆架构

> **to 李金芮 & 王凯：MVP阶段简化为两层（去掉层二切换摘要，因为不存在跨chatflow切换）。**

## 7.1 MVP两层记忆架构

| 层级 | 存储位置 | 生命周期 | 负责人 | 用途 |
|------|---------|---------|--------|------|
| 层一：chatflow内部记忆 | Dify原生会话记忆 | 单次chatflow会话期间 | **to 李金芮**：设置保留5轮 | 对话连贯性 |
| 层二：长期记忆 | `memory_entries` 表 | 永久 | **to 王凯**：新增source和confirmation_count字段 | 跨会话个性化 + 雷达图更新数据源 |

**层二（切换摘要）在MVP阶段不需要。** 原因：MVP只有两种chatflow——盘点系列（02a-02f串联）和主人格。盘点系列内部的板块切换靠板块间摘要提取（已在模块1中设计）。盘点完成后所有日常对话都在主人格一个chatflow里，不存在跨chatflow切换。

> **to 王凯：MVP阶段不需要建session_summaries表。等V2上线引路和发现自由聊天后再建。**

## 7.2 【V2未来迭代】三层记忆架构

> 当多个子智能体chatflow上线后，需要新增层二（切换摘要），用session_summaries表存储30分钟内的chatflow切换上下文。详细设计见V2版本PRD存档。
...
| 组成部分 | Token数 |
|---------|---------|
| system prompt（种子主人格350 / 盘点板块250）| 250-350 |
| memory_injection | 200-500 |
...
> 注：MVP阶段没有session_context（切换摘要），因为不存在跨chatflow切换。V2上线多子智能体后每轮会增加100-150 tokens的切换摘要。

---

# 第九章：数据库核心表

> **to 王凯：本章定义所有需要建的表。标注了MVP（立即建）和V2（未来建）。**
...
age_stage ENUM('exploration','focus','depth')
parent_anxiety_level ENUM('high','medium','low')
entry_path VARCHAR                 -- A/B/C/free
radar_update_pending BOOLEAN DEFAULT false  -- 雷达图是否有待展示的更新
has_talent_radar BOOLEAN DEFAULT false
onboarding_completed BOOLEAN DEFAULT false
last_incomplete_flow VARCHAR       -- 上次未完成的chatflow ID
last_incomplete_step VARCHAR       -- 上次停在哪一步
days_inactive INT DEFAULT 0
total_sessions INT DEFAULT 0
```

## 9.2 child_profiles表（孩子天赋档案——核心表）【MVP】
```sql
CREATE TABLE child_profiles (
  id UUID PRIMARY KEY,
  user_id UUID REFERENCES users(id),
  talent_radar JSONB,              -- 八维度天赋评分（含行为证据和置信度）
  ...
  parent_note TEXT,                -- 基于家长价值观的定制化说明
...
## 9.3 talent_audit_progress表（盘点板块进度）【MVP】
```sql
CREATE TABLE talent_audit_progress (
  id UUID PRIMARY KEY,
  user_id UUID REFERENCES users(id),
  current_block INT DEFAULT 1,     -- 当前板块（1-5）
  block_1_extracted JSONB,         -- 板块一的结构化提取结果
  block_2_extracted JSONB,
  block_3_extracted JSONB,
  block_4_extracted JSONB,
  block_5_extracted JSONB,
...
## 9.4 memory_entries表（修改已有表）【MVP】
```sql
-- 在OPC复用的memory_entries表基础上新增两个字段：
ALTER TABLE memory_entries ADD COLUMN source VARCHAR DEFAULT 'free_chat';
  -- 值：fixed_flow / free_chat / task_feedback
...
-- 新增索引（用于记忆注入服务的查询性能）：
CREATE INDEX idx_memory_user_cat_created ON memory_entries(user_id, category, created_at);
CREATE INDEX idx_memory_user_cat_updated ON memory_entries(user_id, category, updated_at);
```

## 9.5 其他复用表【MVP】
- artifacts, conversation_states, behavior_logs, subscription_tokens —— 完全复用OPC，无需修改

## 9.6 【V2未来迭代】以下表在对应功能上线后再建
```sql
-- growth_tasks表（每周成长任务）—— 引路上线后建
-- mini_projects表（微项目）—— 引路上线后建  
-- session_summaries表（跨chatflow切换摘要）—— 多子智能体上线后建
```
...
|------|-------------|------|------|-----------|-----------|----------|
| 01 | onboarding_flow | 种子 | 固定 | 1 | 无 | 首次登录（Step 1-4暖场→汇聚到"叫什么名字"）|
| 02a | talent_audit_block1 | 发现 | 固定 | 1 | interview_guide + gardner_8_intelligences | 板块一：日常自由时间 |
| 02b | talent_audit_block2 | 发现 | 固定 | 1 | 同上 | 板块二：社交与关系 |
| 02c | talent_audit_block3 | 发现 | 固定 | 1 | 同上 | 板块三：学习与挑战 |
| 02d | talent_audit_block4 | 发现 | 固定 | 1 | 同上 | 板块四：创造与表达 |
| 02e | talent_audit_block5 | 发现 | 固定 | 1 | 同上 | 板块五：家长视角与价值观 |
| 02f | talent_radar_gen | 发现 | 固定 | 1 | ai_era_mapping + talent_radar_template + age_milestones | 五板块摘要汇总→生成雷达图 |
| 03 | zhongzi_main_chat | 种子 | 自由 | 1 | whole_brain_strategies + perma_wellbeing_guide + grit_cultivation + age_adaptation_rules + anxiety_relief_scripts + task_design_principles | **MVP核心日常体验**——育儿教练+信号采集+雷达图更新+闲聊陪伴 |

**MVP总计：8个chatflow，每个1个LLM节点。路由由20行Python完成，零AI成本。**

> **to 李金芮：**
> - 01（onboarding）：1个LLM节点，处理三条暖场路径+自由输入，目标是收集到孩子名字和年龄
> - 02a-02e（五板块访谈）：每个是独立chatflow，共享知识库。板块切换由后端控制——板块结束时触发小模型提取摘要→写入memory_entries→启动下一个板块并注入上下文。Dify会话记忆设5轮。前端用户无感知。
> - 02f（雷达图生成）：输入是五板块的结构化提取汇总（500-800 tokens），输出JSON格式雷达图
> - 03（种子主人格）：**这是MVP阶段用户使用时间最长的chatflow。** 配置6个知识库文档，会话记忆5轮。详细设计见《种子主人格_MVP核心Chatflow详细设计_V1.md》

> **to 史志恒：MVP前端需要支持渲染的卡片类型：**
> - 天赋雷达图（八维度，每个维度含分数+行为证据+AI时代价值说明）
> - 雷达图更新卡片（变化维度绿色↑高亮，旧值→新值对比）
> - 深度访谈进度指示（可选，"我们聊到了第3/5个话题"）
> - 角色切换UI做占位但不实现功能

> **to 王凯：MVP阶段需要建/改的表：**
> - 新建3张：child_profiles, growth_tasks（占位，V2才用）, mini_projects（占位，V2才用）
> - 修改1张：memory_entries（加source VARCHAR + confirmation_count INT）
> - 新增1张：talent_audit_progress（记录每个板块的完成状态和提取结果）
> - 新增字段：users表加 radar_update_pending BOOLEAN DEFAULT false
> - 复用5张：users, artifacts, conversation_states, behavior_logs, subscription_tokens
> - **不需要建**：session_summaries（V2才需要）

---

## 【V2未来迭代】完整chatflow清单

> 以下chatflow在对应子智能体上线后开发。

| 编号 | chatflow ID | 角色 | 说明 | 上线条件 |
|------|-------------|------|------|---------|
| 04 | growth_path_flow | 发现 | 成长路径规划 | 发现子智能体V2 |
| 05 | faxian_free_chat | 发现 | 发现自由聊天（重新盘点/更新方向）| 发现子智能体V2 |
| 06 | weekly_task_flow | 引路 | 每周成长任务生成+反馈收集 | 引路子智能体V2 |
| 07 | mini_project_flow | 引路 | 微项目设计与管理（7岁+）| 引路子智能体V2 |
| 08 | yinlu_free_chat | 引路 | 引路自由聊天 | 引路子智能体V2 |
| 09 | anxiety_relief_flow | 种子 | 独立的焦虑缓释流程（MVP阶段由主人格内置处理）| 用户量达到一定规模后拆分 |
